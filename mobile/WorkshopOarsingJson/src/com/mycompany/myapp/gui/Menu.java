 ...
 * @author wejdene
 */
public class Menu extends Form implements LocalNotificationCallback  {
    Form current;
...
tb.addMaterialCommandToSideMenu("Inscription", FontImage.MATERIAL_HOME, e -> {new addEleve(current).show();}); 
tb.addMaterialCommandToSideMenu("Absence", FontImage.MATERIAL_WEB, e -> {try {
    new listAbsence(current).show();
                } catch (ParseException ex) {
                    
                }
});
tb.addMaterialCommandToSideMenu("Carnets", FontImage.MATERIAL_SETTINGS, e -> {try {
    new affichageCar(current).show();
                } catch (ParseException ex) {
...
tb.addMaterialCommandToSideMenu("Consulter Enfants", FontImage.MATERIAL_INFO, e -> {
                try {
                    new affichageEleve(current).show();
                } catch (ParseException ex) {
                   
                }
});
tb.addMaterialCommandToSideMenu("Rating", FontImage.MATERIAL_INFO, e -> {
    new addRating(current).show();
});
...
addComponent(new Label("Hi World"));
show();
